...
<template>
	<ASelect v-model="filterBy" :hint="`value: ${filterBy}`">
		<li v-for="op in filterByOptions" :key="JSON.stringify(op)" @click="filterBy = op">
			{{ JSON.stringify(op) || typeof op }}
		</li>
	</ASelect>
	<div class="flex items-center gap-6">
		<AInput v-model="q" />
		<ACheckbox v-model="isSearchStrict" label="strict" />
	</div>
	<ATable :rows="results" />
</template>
