	...
	>
		<template #title>
			<div class="flex flex-wrap items-center justify-between">
				...
				<AInput v-model="q" prepend-inner-icon="i-bx-search" class="text-sm max-w-200px" placeholder="Search API..." />
			</div>
		</template>
		...
		<div class="a-card-body">
			...
			<div v-show="apiActiveTab === 'props'">
				<div v-for="prop in filteredProps" :key="prop.name" class="not-last-mb-4">
					...
					<span v-if="prop.default !== 'unknown'" class="text-[hsla(var(--a-base-c),var(--a-text-emphasis-light-opacity))]"> = {{ prop.default }}</span>
					<div class="!children-[p]-m-0" v-html="prop.description" />
				</div>
			</div>

			<!-- Slots -->
			<div v-show="apiActiveTab === 'slots'">
				<div v-for="slot in filteredSlots" :key="slot.name" class="not-last-mb-4">
					...
					<div class="!children-[p]-m-0" v-html="slot.description" />
				</div>
			</div>

			<!-- Events -->
			<div v-show="apiActiveTab === 'events'">
				<div v-for="event in filteredEvents" :key="event.name" class="not-last-mb-4">
