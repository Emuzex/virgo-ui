...
export function useProps(propOverrides?: Partial<ComponentObjectPropsOptions>) {
	...
	if (propOverrides) props = defu(propOverrides, props) as typeof props
...
export function useLayer() {
	...
	const computeClassesStyles = (propColor: ColorProp, propVariant: string, propsStates: boolean, config: UseLayerConfig = {}) => {
		...
		const classes: any[] = [propsStates && _config.statesClass]
    ...
    */
		...
		if (_isThemeColor.value) {
			...
			if (propVariant === 'fill') {
				...
				// Text
				if (propColor !== undefined && propColor !== null) classes.push('text-white')
				...
				// text
				if (propColor !== undefined && propColor !== null) classes.push(`text-${propColor}`)
				...
				// Text
				if (propColor !== undefined && propColor !== null) classes.push('text-[hsl(var(--a-layer-c))]')
			...
			if (propVariant === 'outline') classes.push('border-width-1 border-solid border-current')
		} else if (propColor) {
			...
			if (propVariant === 'fill') {
				...
				// Text
				if (propColor !== undefined && propColor !== null) styles.push({ color: _colord.contrasting().toHslString() })
			} else if (propVariant === 'light') {
				// Background
				styles.push({ background: 'hsla(var(--a-layer-c),var(--a-layer-opacity))' })
				classes.push('[--a-layer-opacity:0.15]')
				...
				// Text
				if (propColor !== undefined && propColor !== null) styles.push({ color: 'hsl(var(--a-layer-c))' })
			} else if (propVariant === 'outline') {
				// Border
				classes.push('border-width-1', 'border-solid')
				styles.push({ borderColor: 'hsl(var(--a-layer-c))' })
	...
	const getLayerClasses = (propColor: MaybeRef<ColorProp>, propVariant: MaybeRef<string>, propsStates: MaybeRef<boolean>, config?: MaybeRef<UseLayerConfig>) => {
		...
		watch(
			[propColor, propVariant, propsStates, () => unref(config)],
			() => {
